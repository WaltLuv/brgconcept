...
const Rentals = () => {
  ...
  const handleSuccess= () => {
    dispatch({
      type: "success",
      message: `Congrats! You have rented a property in ${searchFilters.destination}!!`,
  ...
  const handleNoAccount= () => {
    dispatch({
      type: "error",
      message: `You need to connect your wallet to rent a property`,
      title: "Not Connected",
      position: "topL",
    });
  };
  ...
  useEffect(() => {
    async function fetchRentalsList() {
      const nuRentals = Moralis.Object.extend("nuRentals");
      const query = new Moralis.Query(nuRentals);
  ...
  const bookRental = async function (start, end, id, dayPrice) {
    ...
    let options = {
      contractAddress: "0x49e3c99945424265D378772f4593612ECB88c769",
